...
Source is published at `https://github.com/Omkar142/BankQR`, and the validation build is live at `https://omkar142.github.io/BankQR/` from `gh-pages`. The `/BankQR` deployment path passed a public browser run covering merchant entry, QR generation and customer-page navigation with no resource or console failures. Physical phone scans, banking-app compatibility and merchant timing remain field-validation gates.

UPI intent support is implemented locally after that deployment: an optional merchant VPA creates a strict V2 fragment and an exact `upi://pay` action while V1 links and manual account/IFSC transfer remain supported. Unit and emulated-browser gates pass; publish the new build and validate the chooser on a physical Android phone before calling the UPI flow field-verified.
